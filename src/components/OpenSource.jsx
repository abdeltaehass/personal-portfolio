...
const contributions = [
  ...
  },
  {
    repo: 'uber/NullAway',
    lang: 'Java',
    title: 'Store the nullness analysis in a NullAway field instead of the javac context',
    description: 'A different kind of cleanup in the same tool. NullAway cached its dataflow analysis in the Java compiler\'s shared context, which meant that whichever run reached it first decided the configuration every later run inherited. I moved it onto the checker instance itself, created on first use, matching how the tool already holds the rest of its per-compilation state. The maintainer added a compile-time guard on top so the object can no longer be built the wrong way.',
    pr: 'https://github.com/uber/NullAway/pull/1832',
    prNumber: '#1832',
    additions: 33,
    deletions: 22,
    files: 2,
    commits: 4,
    touched: ['NullAway.java', 'AccessPathNullnessAnalysis.java'],
    merged: 'Sep 2026',
  },
  {
    repo: 'uber/NullAway',
    lang: 'Java',
    title: 'Update Caffeine JMH benchmark to 3.3.0',
    description: 'NullAway measures its own speed by compiling Caffeine, a widely used Java caching library, and that benchmark had been left on the previous release with its compile test switched off. When Caffeine 3.3.0 shipped, the maintainer invited the update: I moved the benchmark over, added the one source file the release introduced, and turned the test back on. The new release compiles cleanly once NullAway\'s experimental JSpecify support is on, which is how Caffeine builds itself, so the benchmark now uses the same setting, with a note that timings before and after aren\'t directly comparable.',
    pr: 'https://github.com/uber/NullAway/pull/1887',
    prNumber: '#1887',
    additions: 5,
    deletions: 5,
    files: 3,
    commits: 2,
    touched: ['build.gradle', 'CaffeineCompiler.java', 'BenchmarkCompilationTest.java'],
    merged: 'Sep 2026',
  },
  {
    repo: 'elastic/eui',
